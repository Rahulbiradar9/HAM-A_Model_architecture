"""
Score m_chat16k_combined_scored.json using Mistral-7B-Instruct-v0.3
Optimized for MAX GPU utilization — batched inference (5 at a time).

Key optimizations:
  - BFloat16 precision (native Ampere/Ada support)
  - Flash Attention 2 via SDPA
  - Batched inference (left-padded) — 5 samples per forward pass
...
BATCH_SIZE = 5             # items per batch — 5 at a time
RETRIES    = 2
SAVE_EVERY = 10            # save checkpoint every N scored items
...
def load_model():
    """Load Mistral-7B-Instruct-v0.3 with max GPU optimization."""
    ...
    # Determine best dtype — RTX 4090 supports BF16 natively
...
def score_batch(entries, tokenizer, model):
    ...
    for entry in entries:
        input_text  = truncate_text(entry.get("input_text", entry.get("input", "")))
        output_text = truncate_text(entry.get("output_text", entry.get("output", "")))
...
def main():
    parser = argparse.ArgumentParser(description="Score m_chat16k_combined_scored with Mistral-7B (GPU-optimized, batched)")
    parser.add_argument("--input-file",  default=r"d:\Rahul_Intern\convo_model\M_chat16k\m_chat16k_combined_scored.json")
    parser.add_argument("--output-file", default=r"d:\Rahul_Intern\convo_model\M_chat16k\m_chat16k_combined_scored_mistral.json")
    parser.add_argument("--batch-size",  type=int, default=BATCH_SIZE, help="Samples per GPU batch (default: 5)")
        ...
        try:
            ...
            for single_entry in batch:
                try:
                    ...
                    input_text  = truncate_text(single_entry.get("input_text", single_entry.get("input", "")))
                    output_text = truncate_text(single_entry.get("output_text", single_entry.get("output", "")))
