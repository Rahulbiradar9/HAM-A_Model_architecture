...
You are scoring a transcript (which could be from a patient or a counselor) using the Hamilton Anxiety Rating Scale (HAM-A).

Your job is to infer and score each HAM-A subscale based on the text provided, and provide a brief reasoning for each score.
Consider:
  - What is EXPLICITLY stated
  - What is IMPLIED through word choice, hedging, or emotional tone
...
1 = Mild: Subtle signs, or patient minimizes
2 = Moderate: Clearly present, somewhat impacts daily life
3 = Severe: Significant impact on daily functioning
...
Return ONLY valid JSON. No explanation outside the JSON.
{
  ...
  "fears": { "score": 0, "reasoning": "..." },
  "insomnia": { "score": 0, "reasoning": "..." },
  "intellectual": { "score": 0, "reasoning": "..." },
  "depressed_mood": { "score": 0, "reasoning": "..." },
  "somatic_muscular": { "score": 0, "reasoning": "..." },
  "somatic_sensory": { "score": 0, "reasoning": "..." },
  "cardiovascular": { "score": 0, "reasoning": "..." },
  "respiratory": { "score": 0, "reasoning": "..." },
  "gastrointestinal": { "score": 0, "reasoning": "..." },
  "genitourinary": { "score": 0, "reasoning": "..." },
  "autonomic": { "score": 0, "reasoning": "..." },
  "behavior_at_interview": { "score": 0, "reasoning": "..." },
  "total_score": 0
}"""

FALLBACK_PROMPT = """Re-evaluate the following text carefully. Your previous assessment returned all zeros, but you may have been too conservative.
Return ONLY valid JSON in the exact same nested format as before (with "score" and "reasoning" for each subscale).
...
def validate_scores(score_dict):
    cleaned = {}
    for s in SUBSCALES:
        sub_data = score_dict.get(s, {})
        if isinstance(sub_data, dict):
            try:
                val = int(round(float(sub_data.get("score", 0))))
            except:
                val = 0
            reasoning = str(sub_data.get("reasoning", ""))
        else:
            try:
                val = int(round(float(sub_data)))
            except:
                val = 0
            reasoning = "Format error, fallback"
            
        val = max(0, min(4, val))
        cleaned[s] = {"score": val, "reasoning": reasoning}

    cleaned["total_score"] = sum(cleaned[s]["score"] for s in SUBSCALES)
    return cleaned
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-file", default=r"d:\Rahul_Intern\convo_model\M_chat16k\m_chat16k_combined_scored_reasoning.json")
    ...
    for i, entry in enumerate(data):
        ...
        if len(input_text.split()) >= 5:
            input_score = get_hama_score(input_text, tokenizer, model)
            
        output_score = None
        if len(output_text.split()) >= 5:
            output_score = get_hama_score(output_text, tokenizer, model)
        ...
        result_entry = {
            ...
            "input_scores": input_score,
            "output_scores": output_score
        }
        
        results.append(result_entry)
        
        in_total = input_score["total_score"] if input_score else "N/A"
        out_total = output_score["total_score"] if output_score else "N/A"
        print(f"  Input Total: {in_total} | Output Total: {out_total}")
